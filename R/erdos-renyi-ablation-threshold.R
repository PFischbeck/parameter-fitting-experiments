...
tbl_fitted <- read.csv("output_data/fitted_features_ablation_threshold/erdos-renyi.csv")
tbl_fitting_process <- read.csv("output_data/fitted_params_ablation_threshold/erdos-renyi.csv")
...
tbl_csv <- tbl %>%
    ...
    ungroup()
write.csv(tbl_csv, "output_data/figures/tables/erdos-renyi_ablation_threshold.csv", row.names = FALSE)
...
addtorow$command <- c(
    "Threshold & \\multicolumn{2}{c}{Mean absolute error} & Iterations \\\\\n",
    " & Vertices & Avg. deg. & \\\\\n"
)
...
print(xtbl,
    ...
    file = "output_data/figures/tables/erdos-renyi_ablation_threshold.tex"
)
