...
tbl_fitted <- read.csv("output_data/fitted_features_ablation_alpha/chung-lu-pl.csv")
tbl_fitting_process <- read.csv("output_data/fitted_params_ablation_alpha/chung-lu-pl.csv")

tbl <- left_join(tbl_fitted, tbl_fitting_process, by = c("graph"), suffix = c("", "_fitting"))

tbl <- tbl %>%
    mutate(graph = sub("_alpha=[0-9\\.]*$", "", graph))

tbl <- tbl %>%
    group_by(graph, alpha) %>%
...
tbl_csv <- tbl %>%
    ...
    ungroup()
write.csv(tbl_csv, "output_data/figures/tables/chung-lu-pl_ablation_alpha.csv", row.names = FALSE)
...
addtorow$command <- c(
    "$\\alpha$ & \\multicolumn{3}{c}{Mean absolute error} & Iterations \\\\\n",
    " & Vertices & Avg. deg. & Het. & \\\\\n"
)


xtbl <- xtable(tbl_print, align = "lrrrrr")
print(xtbl,
    ...
    file = "output_data/figures/tables/chung-lu-pl_ablation_alpha.tex"
)
