...
tbl_fitted <- read.csv("output_data/fitted_features/girg-1d.csv")
# tbl_fitted <- tbl_fitted %>%
#    mutate(graph = sub("_seed=[0-9]*$", "", graph))
tbl_fitting_process <- read.csv("output_data/fitted_params/girg-1d.csv")
...
tbl_generation_params <- read.csv("output_data/attributes/girg-1d.csv")
tbl <- left_join(tbl_generation_params, tbl, by = c("graph"))
tbl <- left_join(tbl, tbl_fitting_process, by = c("graph"), suffix = c("", "_fitting"))

summary(tbl$total_iterations)

# Clustering coefficient
tbl$cc_diff <- abs(tbl$cc_true - tbl$cc_fitted)
cat("Clustering coefficient Pearson correlation:", cor(tbl$cc_true, tbl$cc_fitted, method = "pearson"), "\n")
cat("Clustering coefficient mean absolute difference:", mean(tbl$cc_diff), "\n")
p <- ggplot(tbl, aes(x = cc_true, y = cc_fitted, color = total_iterations)) +
    geom_point(size = 1, stroke = 0, shape = 19) +
    geom_abline(linewidth = 0.1) +
    scale_color_gradient(low = blue, high = red) +
    labs(
        x = "Target clustering coefficient", y = "Mean actual clustering coefficient", color = "Iterations"
    ) +
    theme(legend.position = c(0.2, 0.68))

create_pdf("output_data/figures/figure_girg-1d_difference_cc.pdf", p, width = 0.45, height = 0.5)
...
cat("n absolute difference 90th percentile:", quantile(tbl$n_diff, 0.90), "\n")
...
create_pdf("output_data/figures/figure_girg-1d_difference_n.pdf", p, width = 0.45, height = 0.5)
...
create_pdf("output_data/figures/figure_girg-1d_difference_heterogeneity.pdf", p, width = 0.45, height = 0.5)
...
create_pdf("output_data/figures/figure_girg-1d_difference_d.pdf", p, width = 0.45, height = 0.5)
...
# Mean iterations
cat("Iterations 90th percentile:", quantile(tbl$total_iterations, 0.90), "\n")
