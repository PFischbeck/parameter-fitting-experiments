...
tbl_fitted <- read.csv("output_data/fitted_features_ablation_alpha/girg-1d.csv")
tbl_fitting_process <- read.csv("output_data/fitted_params_ablation_alpha/girg-1d.csv")
...
tbl_csv <- tbl %>%
    ...
    ungroup()
write.csv(tbl_csv, "output_data/figures/tables/girg-1d_ablation_alpha.csv", row.names = FALSE)
...
addtorow$command <- c(
    "$\\alpha$ & \\multicolumn{4}{c}{Mean absolute error} & Iterations \\\\\n",
...
print(xtbl,
    ...
    file = "output_data/figures/tables/girg-1d_ablation_alpha.tex"
)
