...
tbl_fitted <- read.csv("output_data/fitted_features_ablation_alpha/erdos-renyi.csv")
tbl_fitting_process <- read.csv("output_data/fitted_params_ablation_alpha/erdos-renyi.csv")
...
tbl_csv <- tbl %>%
    ...
    ungroup()
write.csv(tbl_csv, "output_data/figures/tables/erdos-renyi_ablation_alpha.csv", row.names = FALSE)
...
addtorow$command <- c(
    "$\\alpha$ & \\multicolumn{2}{c}{Mean absolute error} & Iterations \\\\\n",
    " & Vertices & Avg. deg. & \\\\\n"
)

xtbl <- xtable(tbl_print, align = "lrrrr")
print(xtbl,
    ...
    file = "output_data/figures/tables/erdos-renyi_ablation_alpha.tex"
)
